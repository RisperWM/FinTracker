...
import { format, subDays, isSameDay, parseISO, startOfDay, differenceInDays, differenceInWeeks, differenceInMonths } from "date-fns";
...
const CATEGORIES = [
    ...
    ...Object.keys(CATEGORY_COLORS).map(key => ({ label: key, color: CATEGORY_COLORS[key] }))
];
...
export const HabitList = ({ searchQuery, activeCategory, setActiveCategory }: HabitListProps) => {
    const { habits, fetchHabits, loading, logs, deleteHabit } = useHabitStore();
    ...
    const calculateProgress = (habit: HabitType) => {
        ...
        if (freq === "Monthly") {
            unitLabel = "Month";
            diff = differenceInMonths(today, start) + 1;
            total = end ? differenceInMonths(end, start) + 1 : 0;
        } else if (freq === "Weekly") {
            unitLabel = "Week";
            diff = differenceInWeeks(today, start) + 1;
            total = end ? differenceInWeeks(end, start) + 1 : 0;
        } else {
            unitLabel = "Day";
            diff = differenceInDays(today, start) + 1;
            total = end ? differenceInDays(end, start) + 1 : 0;
        }

        diff = Math.max(0, diff);
        if (total > 0) diff = Math.min(diff, total);
        const percentage = total > 0 ? Math.round((diff / total) * 100) : 0;

        return { percentage, current: diff, total: total > 0 ? total : '∞', unitLabel };
    };
    ...
    const renderHabitCard = ({ item }: { item: HabitType }) => {
        ...
        return (
            ...
            >
                ...
                <View style={[styles.innerContent, { backgroundColor: isSelected ? "#fffcf5" : "#ffffff" }]}>
                    ...
                    <View style={styles.textContainer}>
                        ...
                        <Text style={styles.description}>
                            {progress.unitLabel} {progress.current} of {progress.total} • {item.frequency}
                        </Text>
                    </View>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryScroll}>
                        {CATEGORIES.map((cat) => (
                            <TouchableOpacity
                                ...
                                style={[styles.categoryPill, activeCategory === cat.label && { backgroundColor: cat.color }]}
                            >
                                <Text style={[styles.categoryText, activeCategory === cat.label && { color: '#fff' }]}>
                                    {cat.label}
                                </Text>
                            </TouchableOpacity>
                        ))}
...
const styles = StyleSheet.create({
    ...
    categoryPill: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: '#fff', borderWidth: 1, borderColor: '#e2e8f0' },
    ...
    innerContent: { flex: 1, flexDirection: "row", alignItems: "center", marginLeft: 5, paddingVertical: 12, paddingHorizontal: 14, borderTopLeftRadius: 10, borderBottomLeftRadius: 10 },
